...
{
    public RaidLeaderProcEventHandler(MaxLifeCalculator maxLifeCalculator) => _maxLifeCalculator = maxLifeCalculator;
    ...
    public override void ProcessEvent(RaidLeaderProcEvent e, SimulationState state)
    {
        ...
        {
            var healEvent = new HealingEvent(e.Timestamp + i + 1, _maxLifeCalculator.Calculate(state) * RaidLeader.GetHealPercentage(state));
